...
router.get('/overview', auth, async (req: Request, res: Response) => {
  try {
    ...
    // Get analytics stats
    const userStats = await Analytics.getUserStats(userId, parseInt(period as string));
    ...
    const topTemplates = await Analytics.getTemplatePerformance(userId, parseInt(period as string));
...
router.get('/timeseries', auth, async (req: Request, res: Response) => {
  try {
...
router.get('/templates', auth, async (req: Request, res: Response) => {
  try {
    ...
    const templatePerformance = await Analytics.getTemplatePerformance(userId, parseInt(period as string));
...
router.get('/channels', auth, async (req: Request, res: Response) => {
  try {
    ...
    const channelStats = await Analytics.getChannelStats(userId, parseInt(period as string));
...
router.get('/engagement', auth, async (req: Request, res: Response) => {
  try {
    const { period = 30 } = req.query;
    const user = req.user;
