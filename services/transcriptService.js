...
const { generateTextEmbedding, summarizeTranscript, extractKeyTopics } = require('./openaiService');

const sentimentAnalyzer = new sentiment();
const tokenizer = new natural.WordTokenizer();

const extractTranscript = async (videoId, userId, channelId) => {
  try {
    console.log(`Extracting transcript for video: ${videoId}`);

    const existingTranscript = await VideoTranscript.findOne({ videoId });
    if (existingTranscript && existingTranscript.isProcessed) {
      return {
        success: true,
    ...
    const processedSegments = transcriptData.map(segment => ({
...
const getTranscriptByVideoId = async (videoId) => {
  try {
...
const getTranscriptsByUser = async (userId, options = {}) => {
  try {
    const {
      channelId,
      limit = 10,
    ...
    const query = { user: userId, isProcessed: true };
...
const searchTranscripts = async (userId, query, options = {}) => {
  try {
...
const deleteTranscript = async (videoId, userId) => {
  try {
...
const getTranscriptStats = async (userId) => {
  try {
