...
const { getChannelVideos, getVideoComments, replyToComment, refreshAccessToken } = require('./youtubeService');
...
class CommentMonitorService {
  ...
  async monitorUserComments(user) {
      ...
      try {
        ...
        if (channelError.code === 401 && channel.refreshToken) {
          try {
            ...
            const channelIndex = user.youtubeChannels.findIndex(ch => ch.channelId === channel.channelId);
            if (channelIndex >= 0) {
