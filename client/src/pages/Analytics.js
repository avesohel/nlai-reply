            ...
            {sentimentData ? (
              <div className="space-y-4">
                {Object.entries(sentimentData).map(([sentiment, data]) => {
                  ...
                  const total = Object.values(sentimentData).reduce((acc, curr) => acc + (curr.count || 0), 0);
                  const percentage = total > 0 ? ((data.count || 0) / total) * 100 : 0;

                  return (
                    <div key={sentiment}>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-sm font-medium text-gray-900 capitalize">{sentiment}</span>
                        <span className="text-sm text-gray-600">{data.count || 0} ({percentage.toFixed(1)}%)</span>
                      </div>
